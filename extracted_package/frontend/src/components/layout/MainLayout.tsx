...
export const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  const adminNavigation = user?.profile?.is_staff ? [
              ...
              <div className="relative">
                ...
                >
                  <img
                    ...
                    src={user?.profile?.avatar_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.username || 'User')}&background=3b82f6&color=fff`}
                    alt=""
                  />
                  <span className="hidden lg:flex lg:items-center">
                    <span className="ml-4 text-sm font-semibold leading-6">
                      {user?.profile?.first_name ? `${user.profile.first_name} ${user.profile.last_name || ''}` : user?.username || 'User'}
                    </span>
                    <svg
